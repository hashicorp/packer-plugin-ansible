...
- `galaxy_force_with_deps` (bool) - Force overwriting an existing role
    and its dependencies. Adds `--force-with-deps` option to `ansible-galaxy`
    command. By default, this is `false`.
