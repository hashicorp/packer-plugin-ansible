...
func (p *Provisioner) Prepare(raws ...interface{}) error {
	...
	if p.config.RolesPath != "" {
		if p.config.GalaxyRolesPath != "" {
			errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("`roles_path` and `galaxy_roles_path` are mutually exclusive and cannot be specified in the same configuration"))
		}
		p.config.GalaxyRolesPath = p.config.RolesPath
	}

	if p.config.CollectionPaths != "" {
		if p.config.GalaxyCollectionsPath != "" {
			errs = packersdk.MultiErrorAppend(errs, fmt.Errorf("`collections_path` and `galaxy_collections_path` are mutually exclusive and cannot be specified in the same configuration"))
		}
		p.config.GalaxyCollectionsPath = p.config.CollectionPaths
	}
...
func (p *Provisioner) executeAnsible(ui packersdk.Ui, comm packersdk.Communicator, privKeyFile string) error {
	...
	if p.config.GalaxyFile != "" {
		args, err := ansiblecommon.BuildGalaxyArgs(
			filepath.ToSlash(p.config.GalaxyFile),
